...
class PulserDummy(PulserInterface):
    ...
    def on_activate(self):
        ...
        # yy = np.sin(2 * np.pi * 10 * xx)  # Example waveform
        # qdplot.plot1d(xx, yy, label='Pulser Dummy Waveform')
        print("[PulserDummy:on_activation]: PulserDummy was Activated")
    ...
    def pulser_on(self):
        ...
        """
        print("[PulserDummy:pulser_on]: PulserDummy was turned On.")
    ...
    def write_waveform(self, name, analog_samples, digital_samples, is_first_chunk, is_last_chunk,
        ...
        """
        print("[PulserDummy:write_waveform]: Writing waveform.")
        print("[PulserDummy:write_waveform]: Input Analog Samples: {0}".format(analog_samples))
        print("[PulserDummy:write_waveform]: Input Digital Samples: {0}".format(digital_samples))
        
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        
        plt.figure()
        plt.plot(analog_samples['a_ch1'], '-', label='Analog Channel 1')
        plt.plot(analog_samples['a_ch2'], '-', label='Analog Channel 2')
        plt.legend()
        plt.savefig(os.path.expanduser(f"~/Desktop/KIT/SS25/DiamondBaby/a_ch_waveform_{name}_{timestamp}.pdf"))
        
        plt.figure()
        plt.plot(digital_samples['d_ch1'], '-', label='Digital Channel 1')
        plt.plot(digital_samples['d_ch2'], '-', label='Digital Channel 2')
        plt.plot(digital_samples['d_ch3'], '-', label='Digital Channel 3')
        plt.plot(digital_samples['d_ch4'], '-', label='Digital Channel 4')
        plt.legend()
        plt.savefig(os.path.expanduser(f"~/Desktop/KIT/SS25/DiamondBaby/d_ch_waveform_{name}_{timestamp}.pdf"))
        
        print("[PulserDummy:write_waveform]: Saved waveform plot as 'a_ch' / 'd_ch'.")
        
        with open(os.path.expanduser("~/Desktop/KIT/SS25/DiamondBaby/analog_channel.txt"), "w", encoding="utf-8") as f:
            for key, arr in analog_samples.items():
                arr = np.asarray(arr)  # falls schon Liste
                f.write(f"{key}: " + " ".join(map(str, arr)) + "\n")
        
        with open(os.path.expanduser("~/Desktop/KIT/SS25/DiamondBaby/digital_channel.txt"), "w", encoding="utf-8") as f:
            for key, arr in digital_samples.items():
                arr = np.asarray(arr)  # falls schon Liste
                f.write(f"{key}: " + " ".join(map(str, arr)) + "\n")

        print("[PulserDummy:write_waveform]: Saved waveform file.")
    ...
    def load_waveform(self, load_dict):
        ...
        print("[PulserDummy:load_waveform]: Loading waveform.")
        print("[PulserDummy:load_waveform]: Load Dict: {0}".format(load_dict))
        ...
        print("[PulserDummy:load_waveform]: Return Dict: {0}".format(self.get_loaded_assets()[0]))
